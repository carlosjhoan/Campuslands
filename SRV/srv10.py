...
def valid_id(lst, msj):
    while True:
        try:
            ...
            if a == 1:
                print ("\nEste ID no se puede asignar. Ya se encuentra en la base de datos")
                continue
...
while True:
    ...
    if opc == 1:
        ...
        empleado["horas_trabajadas"] = hor_trab
        val_hora = valor_hora("Ingrese el valor de la hora del empleado: ")
        empleado["valor_hora"] = val_hora
        ...
        opc_modif = menu_modif("""Qué desea modificar?
                                ...
                                4 -- Dejar todo igual""")
        if opc_modif == 1:
            list_empleados[pos_empl]["nombre"] =  valid_nombre("Ingrese el nombre del empleado: ")

        elif opc_modif == 2:
            list_empleados[pos_empl]["horas_trabajadas"] =  horas_trabajadas("Ingrese la cantidad de horas trabajadas por el empleado: ")
        
        elif opc_modif == 3:
            list_empleados[pos_empl]["valor_hora"] = valor_hora("Ingrese el valor de la hora del empleado: ")
        ...
        for i in list_empleados:
            if i["id"] == id_buscar:
                ...
                print (f"ID: {i['id']}")
                print (f"Horas trabajadas: {i['horas_trabajadas']}")
                print (f"Valor de la hora: ${i['valor_hora']:,}")
                print ( "*=" * 30)
                a = 1
        ...
        for i in list_empleados:
            if i["id"] == id_buscar:
                ...
                print (f"Horas trabajadas: {i['horas_trabajadas']}")
                print (f"Valor de la hora: ${i['valor_hora']:,}")
                print (f"Descuentos por EPS y pensión: ${nom_empleado (i['horas_trabajadas'], i['valor_hora'])[1]:,.0f}")
                print (f"Nómina: ${nom_empleado (i['horas_trabajadas'], i['valor_hora'])[0]:,.0f}")
                print ( "*=" * 30)
                a = 1
        ...
        nom_total = 0
        for i in list_empleados:
            nom_total += nom_empleado (i['horas_trabajadas'], i['valor_hora'])[0]
        print (f"La nómina total de los empleados es: ${nom_total:,.0f}")
        input ("\nPresione cualquier tecla para volver al menú principal...")
    
    else:
        s_n = input("""Desea salir? 
                    s -- Sí, deseo salir
        ...
        if s_n == "s" or s_n == "S":
            ...
            input ("Presiona cualquier tecla para Scontinuar... ")
